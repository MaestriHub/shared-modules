...
object CasePriceSerializer : KSerializer<Procedure.Helpers.CasePrice> {
    override val descriptor: SerialDescriptor = PrimitiveSerialDescriptor("CasePrice", PrimitiveKind.STRING)

    override fun serialize(encoder: Encoder, value: Procedure.Helpers.CasePrice) {
        var digit: BigDecimal = BigDecimal.ZERO
        var type: String = ""

        when(value) {
            is Procedure.Helpers.CasePrice.FixedValue -> {
                digit = value.value
                type = "fixedValue"
            }
            is Procedure.Helpers.CasePrice.MultiKoeff -> {
                digit = value.value
                type = "multiKoeff"
            }
        }
    ...
    override fun deserialize(decoder: Decoder): Procedure.Helpers.CasePrice {
        ...
        return when (type) {
            "fixedValue" -> Procedure.Helpers.CasePrice.FixedValue(digit)
            "multiKoeff" -> Procedure.Helpers.CasePrice.MultiKoeff(digit)
